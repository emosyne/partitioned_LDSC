...
(results <- data.table::fread("~/GoogleDrive/WORK/CF_PhD/NF_2HH/partitioned_LDSC/2023_01_30_NEURAL_GRBorNot_100flank.results") %>% 
...
ggplot(aes(y=Enrichment, x = forcats::fct_rev(forcats::fct_inorder(Category)), fill = Enrichment_p), data= results)+
  ...
  geom_text(aes(label = paste0("Enrichment = ",round(Enrichment,2),", p value = ",round(Enrichment_p,2), 
...
##CARDIAC
(results <- data.table::fread("~/GoogleDrive/WORK/CF_PhD/NF_2HH/partitioned_LDSC/2023_01_30_CARDIAC_noFibro_GRBorNot_100flank.results") %>% 
    replace_na(list(Enrichment_p=1)) )#%>% 
#pivot_longer(cols=!Category) )
# table(results$Category)
results$Category <- sub("L2_0", "", results$Category)
...
(results <- results %>% select( c("Category","Enrichment","Enrichment_p")) %>% 
    filter(grepl("base|Coding|Intron|Promoter|Enhancer|UTR|GRB|FANTOM|Negative|Cardiac|Heart", Category)) %>% 
    filter(!grepl("extend", Category))
)
